...
原生打开/添加文件夹/保存对话框在 Rust 侧登记用户实际选择的路径；文本和二进制写入只允许落在这些会话级授权范围内，并使用临时文件替换，避免先删除原文件造成空窗。读取命令、工作区监听权限和全局 `**` 作用域仍需要继续收紧到用户明确选择的目录；超大文档和增量索引也会在后续版本继续推进。不要把完整 `App.tsx` 拆分或跨平台安装包当作已完成能力。
