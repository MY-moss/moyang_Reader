...
原生打开/添加文件夹/保存对话框在 Rust 侧登记用户实际选择的路径；读取和写入使用独立的会话级授权范围。单独打开文档时，读取范围包含其所在目录以支持相对图片和附件，但写入范围仍只包含用户明确选择的文件；选择工作区文件夹后才授予该工作区的读写范围。写入使用临时文件替换，避免先删除原文件造成空窗。PDF、图片和相对附件通过授权后的二进制读取转换为内存 Blob URL，不再依赖全局 `asset://**` 作用域；工作区监听仍使用插件权限，后续需要按工作区继续收紧。超大文档和增量索引也会在后续版本继续推进。不要把完整 `App.tsx` 拆分或跨平台安装包当作已完成能力。
