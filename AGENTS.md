...
当前阶段优先完成 v0.11/v0.12 的结构、桌面体验、性能、安全与发布验证，不继续扩大产品表面。

在 `docs/ROADMAP.md` 的收敛条件满足前：

- 不提前实现 Reading Inbox、Daily Note、Properties 表格、AI provider、RAG、RSS、插件市场或新的远程内容入口；这些能力可以分析、记录 Issue 或完善长期设计，但默认不编码、不创建实现 PR。
- v0.11 当前任务继续严格按 `AI-TASKS.md` 顺序推进，优先完成 A07 → A08 → A09 → A10；之后只追加与职责拆分、RC 收口和真实 Windows 发布验证直接相关的小切片。
- 新代码不得重新把职责堆回 `App.tsx`、`commands.rs` 或其他大型编排中心；提取必须按稳定业务职责进行，并保持行为等价与可回滚。
- `main` 与稳定 Release 之间存在较多结构/UI 改动时，真实 Windows 主流程、安装/升级、PDF 落盘、更新器和签名边界验证优先于任何新功能。
- 旧 AI governance policy/state machine、额外任务板、身份隔离与生成式审批流程均视为历史设计；当前唯一开发控制面是本文件、`docs/AI-TASKS.md`、`docs/ROADMAP.md` 与架构契约。不得因为历史文档仍保留相关描述而恢复已废弃流程。
