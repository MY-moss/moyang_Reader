...
- 计划中明确记录的已取消任务只保留历史原因，不计入 completedTaskIds；AI 只能按状态机跨过它们，不能自行新增或删除取消记录。本次 G03 取消是一次已记录的用户计划修订，不是普通任务先例。
...
3. 只有当前任务允许自动交付、风险不高于 T2、没有保护文件变更且所有依赖已完成或按计划取消时，才可提交、推送、开 PR 和自动合并。
