...
- 当前证据：PR #531 已合入最新 `main`；Issue #16 阶段 2 仍开放；工作区搜索生命周期已由 `workspace-search-controller` 持有；当前没有修改同一范围的开放产品 PR，开放 PR 仅为无关 Dependabot。
...
- 结果：主题与阅读外观由 #536 落地；右栏、顶栏、标签/状态栏及阅读画布的后续独立切片由 #537–#540 落地，均已通过对应浏览器、无障碍和 Windows CI。
- 下一项：D17 文件树操作状态收口，再迁移左栏界面。
...
- 关联：#534 / #16
- 目标：提取 `WorkspaceEntryOperationsController`，统一重命名、删除、移动和复制完成后的当前文档、标签、最近文件、工作区会话与缓存更新，为左栏迁移建立稳定边界。
- 范围：保留现有 Rust/IPC、确认文案、文件真源和快捷键；只有文件操作成功后才提交会话变化，工作区切换期间不覆盖新工作区视图。
- 验收：取消、保存失败、文件失败、过期操作、重新打开失败均无半更新或静默丢稿；相关单测、桌面 smoke 和远程 CI 通过。
- 结果：文件操作只在原有 Rust 调用成功后提交会话变化；删除失败不再提前释放资源；过期操作保留新工作区视图与错误状态。中英提示、定向单测及 Windows 桌面 smoke 随 PR 验证。
- 下一项：D18 左栏文件导航与按需搜索/筛选迁移。

### D18 — 左栏文件导航与按需搜索/筛选迁移

**状态：DONE — PR #542**

- 关联：#534
- 目标：让文件树成为常驻主体，将工作区搜索和筛选按需展开、阅读历史独立折叠；工作区管理与批量导出进入工作区菜单。
- 范围：保持现有文件操作控制器、IPC、快捷键与三栏信息架构；同步中英文文案及 1240/900/720 窗口状态。
- 验收：打开文件、搜索、筛选、历史与批量导出的必要步骤不增加；键盘焦点和抽屉收起行为可预测；浏览器、无障碍和 Windows 桌面 smoke 通过。
- 结果：文件树保留为左栏主体；搜索/筛选在原快捷键可直达的前提下按需展开，历史独立折叠，阅读库切换/管理/批量导出进入同一菜单。迁移中英文文案，并验证 518 项单测、工作区与无障碍浏览器场景及 18 项 Windows 桌面 smoke。
- 下一项：D19 编辑器工具条、插入菜单与上下文菜单统一迁移。
...
- 关联：#534 / #111 / #171
- 目标：让源码与所见即所得编辑器的工具条、插入菜单和上下文菜单遵循同一套控件尺寸、状态反馈、键盘规则及中英文文案。
- 范围：只改编辑器表层和交互呈现；保留文档格式、保存/恢复、命令 ID、快捷键、IPC 与现有编辑语义。
- 验收：两种编辑模式的格式化、插入、右键操作无步骤回退；720/900/1240 窗口与浅/深/高对比状态无溢出；键盘焦点、无障碍、浏览器和 Windows 桌面 smoke 通过。
- 结果：两种模式继续复用工具条、插入面板和上下文菜单；迁移完整中英文案及错误提示，统一主要控件至至少 32px，紧凑窗口保留清除格式动作。验证 521 项单测、113 项浏览器场景、18 项 Windows 桌面 smoke；编辑器浅/深色视觉基线随实际界面更新。
- 下一项：D20 设置与首次使用界面迁移。
...
- 关联：#534 / #111 / #171
- 目标：让设置、首次使用指引与空状态遵循 v1.0 的组件尺寸、信息层级和中英文规范。
- 范围：只迁移呈现与焦点/展开交互；保留设置存储 key、迁移语义、默认离线和文件安全边界；确认、冲突、草稿、更新与导出弹层留给后续独立切片。
- 验收：设置查找与更改、首次打开阅读库的必要步骤不增加；720/900/1240、三套主题、键盘与高对比状态可用；单测、浏览器、无障碍及 Windows 桌面 smoke 通过。
