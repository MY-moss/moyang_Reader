...
- 更新端点按配置顺序先尝试 GitHub Release 权威源，再回退到公开 Cloudflare Pages 动态镜像。动态镜像可访问不等于静态镜像工作流成功；静态部署必须由 `mirror-release.yml` 使用 Release 资产和 Cloudflare Secret 完成，并由 `mirror-health.yml` 巡检。
