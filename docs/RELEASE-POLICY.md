...
当前稳定基线：`v0.10.12`。该版本的 tag、Windows x64 安装包、签名和 manifest 已在线核验；公开 Pages 镜像资产也已核验，但静态镜像部署 job `98973037391` 仍因缺少 `CLOUDFLARE_API_TOKEN` 与 `CLOUDFLARE_ACCOUNT_ID` 失败，不得误判为整个 Release workflow 全绿。
