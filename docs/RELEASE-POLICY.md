...
当前稳定基线：`v0.10.10`；该版本已完成 Windows x64 Release、安装包、签名和 manifest 在线核验。公开 Pages 镜像代理资产已核验一致；静态镜像部署 job 仍因 Cloudflare Secret 缺失失败，不能误判为静态自动同步已全绿。下一发布目标按 Ready backlog 决定。
