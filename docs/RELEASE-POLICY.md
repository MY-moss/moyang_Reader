...
当前稳定基线：`v0.9.4`；本版本包含 #183 标签页文档缓存，并已完成版本提交合并、Windows 发布 workflow、GitHub Release、签名资产、Cloudflare 公开资产和旧版自动更新验证。Release 镜像子任务仍因仓库尚未配置 `CLOUDFLARE_API_TOKEN` / `CLOUDFLARE_ACCOUNT_ID` 而失败；不能把公开资产在线误判为自动同步 workflow 全绿，剩余问题由 #241 跟踪。
