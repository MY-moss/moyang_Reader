...
- 下一位 AI 的唯一下一步：先查看 Issues 和本文件，确认 #185 已完成、#218 已合并；#88 本阶段已完成但保持 open，优先处理 PDF 文件落盘或更新器桌面回归，随后再处理 #169 的工作区规模基线。仍需补做一次 Windows 旧版本点击更新、签名校验、替换安装、重启后的版本号回归。#170 的代码、回归和 Release 在线核验已完成并已关闭；#180 已由 PR #210 合并并关闭；#177 的防抖实现已由 PR #208 合并并关闭。不要重复实现当前的搜索高亮、阅读位置、外部修改保护、WYSIWYG 渲染防抖、启动/编辑/保存/无冲突外部刷新/外部新增删除/HTML+Word 导出/关闭确认 smoke，也不要把 #174（React 错误边界）混入下一切片。
