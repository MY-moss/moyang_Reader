...
- 下一位 AI 的唯一下一步：先补旧版本 `v0.8.1` 到 `v0.8.2` 的真实更新安装/重启回归；随后选择 #88 的 PDF 文件落盘、更新/双链与 `/` 真实桌面回归，或 #169 的工作区规模基线。#177 的防抖实现已完成，待当前 PR 合并后按评论记录并关闭；不要重复实现当前的外部修改保护、启动/编辑/保存/无冲突外部刷新/外部新增删除/HTML+Word 导出/关闭确认 smoke，也不要把 #174（React 错误边界）混入本切片。
