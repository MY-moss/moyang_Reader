...
- 已实现：相关 Tauri 命令统一通过 `spawn_blocking` 执行；搜索缓存改为可安全共享的 `Arc` 状态；工作区文件快照加入修改时间；删除目录事件保留缺失目录本身的失效范围；前端按文件/文件夹快照决定是否复用索引；Windows 普通路径、`\\?\\` 扩展路径和 UNC 路径统一比较。
...
- 已完成验证：前端定向测试 6/6、前端全量测试 217/217、浏览器 E2E 45/45、Rust 全量测试 47/47、Lint、格式检查、前端构建和 Rust clippy 已通过；Windows 桌面用例在修复前 10/11 通过，修复删除目录事件后单项 smoke 1/1 通过；扩展路径归一化定向测试 1/1 通过。最终门禁完成后补录 PR、合并提交和 Release 状态。
