...
5. 只有远程状态已确认且前序无真实阻塞时，才从最新 main 建一个 `codex/` 分支，只完成一个垂直切片；当前最早可执行任务是 `D13` 工作区搜索生命周期编排提取，不要跳过任务顺序。
