...
- 下一位 AI 的唯一下一步：先查看 Issues 和本文件，确认 `v0.8.3` 发布链路已完成；优先选择 #185/#186 的数据安全与键盘交互闭环，或 #88 的 PDF 文件落盘/更新/双链与 `/` 真实桌面回归，或 #169 的工作区规模基线。仍需补做一次 Windows 旧版本点击更新、签名校验、替换安装、重启后的版本号回归。#170 的代码、回归和 Release 在线核验已完成并已关闭；#180 已由 PR #210 合并并关闭；#177 的防抖实现已由 PR #208 合并并关闭。不要重复实现当前的搜索高亮、阅读位置、外部修改保护、WYSIWYG 渲染防抖、启动/编辑/保存/无冲突外部刷新/外部新增删除/HTML+Word 导出/关闭确认 smoke，也不要把 #174（React 错误边界）混入下一切片。
