...
- 当前工程主线是：A14 v0.11.0 已发布 → B01–B06 v0.12 可靠性/真实使用切片完成 → C01–C04 v1.0 Freeze/Compatibility/RC 收口完成 → D00 事实源与接手文档收口完成 → D01 开发环境与接手入口 → D02 开发环境自检完成 → D03 标准首次运行验证路径完成 → D04 v1.0 Freeze 执行契约完成 → D05 架构复杂度预算完成 → D06 App.tsx 文内查找编排提取完成 → D07 App.tsx 阅读位置恢复与保存编排提取完成 → D08 App.tsx 阅读进度栏与标题观察编排提取完成 → D09 App.tsx 更新生命周期编排提取完成 → D10 App.tsx 批注高亮生命周期编排提取完成 → D11 App.tsx 设置与偏好生命周期编排提取完成 → D12 App.tsx 命令编排提取完成 → D13 App.tsx 工作区搜索生命周期编排提取完成 → D14 App.tsx 路径入口编排提取进行中；A07–A13 的仓库内收口和 v0.12 Exit Gate 证据已完成。
...
5. 只有远程状态已确认且前序无真实阻塞时，才从最新 main 建一个 `codex/` 分支，只完成一个垂直切片；当前最早可执行任务是 `D14` 路径入口编排提取，不要跳过任务顺序。
