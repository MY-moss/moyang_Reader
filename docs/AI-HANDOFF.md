...
- 当前工程主线是：A14 v0.11.0 已发布 → B01–B06 v0.12 可靠性/真实使用切片完成 → C01–C04 v1.0 Freeze/Compatibility/RC 收口完成 → D00–D14 事实源、开发环境、架构预算与 App 编排提取完成 → D15 v1.0 重设计 P1 质量清障完成 → D16 三主题与阅读外观生产切片进行中（PR #536）→ 应用壳与面板分片迁移 → v1.0 RC；总票为 #534。
...
5. 只有远程状态已确认且前序无真实阻塞时，才从最新 main 建一个 `codex/` 分支，只完成一个垂直切片；当前最早可执行任务是 `D16` 三主题与阅读外观生产切片，继续 PR #536 并完成真实浏览器视觉验收。
