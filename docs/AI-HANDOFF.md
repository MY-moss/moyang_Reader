...
- 当前工程主线是：A14 v0.11.0 已发布 → B01–B06 v0.12 可靠性/真实使用切片完成 → C01–C04 v1.0 Freeze/Compatibility/RC 收口完成 → D00–D14 事实源、开发环境、架构预算与 App 编排提取完成 → D15 v1.0 重设计 P1 质量清障完成 → D16 Figma 设计真源等待连接重新认证 → 分片生产 UI 迁移 → v1.0 RC；总票为 #534。
...
5. 只有远程状态已确认且前序无真实阻塞时，才从最新 main 建一个 `codex/` 分支，只完成一个垂直切片；当前最早可执行任务是 `D16` v1.0 Figma 设计真源，完成 Figma 连接重新认证后继续，不要跳过设计审核进入生产 UI。
