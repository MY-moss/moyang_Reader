...
## 进行中功能切片（2026-08-28，v0.10.9 右键快速管理补全）

- 目标：补齐文件树和阅读正文的高频右键动作，让文件/文件夹的查看、管理、定位与文本操作形成闭环。
- 分支：`codex/context-menu-more-2026-08-28`；本地基于既有 v0.10.8 工作区继续开发，远程提交应以最新 `main` 为基线创建独立 PR。
...
- 验收：WorkspaceTree/ReaderContextMenu/编辑器动作定向测试、类型检查、Lint、Prettier、阅读模式浏览器 E2E 通过；完整构建已在本切片前段通过，最终类型检查已复核。
- 交接：提交后创建唯一 PR 关联 #232（先使用 `Refs #232`，待远程验收完整后再决定关闭）；合并前不创建 Release，稳定批次再准备 v0.10.9 Windows x64 安装包、签名、manifest 和镜像核验。若继续工作，先检查 PR 状态，不扩大到文件移动或批量操作。
