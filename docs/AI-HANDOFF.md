...
## 当前功能切片（2026-08-28，v0.10.10 工作区转移与纯文本粘贴）

- 目标：补齐文件/文件夹右键的工作区内剪切、复制、粘贴，并完善 Markdown/TXT/WYSIWYG 的纯文本粘贴动作。
- 分支：`codex/context-menu-transfer-2026-08-28`，基于稳定 `main` 的 v0.10.9；代码提交 `4c765169d6993939250261fe2870f25e165dac59` 已推送，PR 尚待创建。
- 已实现：Tauri 端在授权工作区内安全复制/移动文件和文件夹；文件树支持剪切/复制并粘贴到根目录或其他文件夹，阻止重名和自身/子目录目标；移动后同步打开标签页、当前文档、最近记录和缓存路径；Markdown/TXT/WYSIWYG 增加“粘贴为纯文本”，复制可重复粘贴、移动后清空待粘贴状态。
- 验证：`npm run format:check`、`npm run lint`、前端定向 Vitest（2 个文件、12 个用例）、`cargo fmt --check`、Rust 转移定向测试（1/1）和一次 `npm run build` 通过；构建仅保留既有入口 chunk size warning。
- 非目标：跨工作区移动、回收站恢复、批量操作、覆盖粘贴、属性写回、DOCX/PDF 原格式编辑和云同步不在本切片内。
- 下一步唯一动作：推送最终提交并创建一个 PR；PR 合并且门禁全绿后准备 `v0.10.10` 版本和 Windows x64 安装包。不要在本切片中顺手开始其他功能。
