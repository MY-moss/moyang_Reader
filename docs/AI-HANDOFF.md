...
- 在线下一步：网络已恢复，使用真实 Git 推送一次交接文档更新以触发 `Quality checks`；通过且无真实冲突后合并。合并后只执行一次稳定发布：更新版本号为 `v0.9.1`，生成 Windows x64 安装包、`.sig`、`latest.json`，同步 Cloudflare 镜像并验证旧版本更新链路。
