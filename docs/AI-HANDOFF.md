...
- 下一位 AI 的唯一下一步：先查看 Issues 和本文件，确认本镜像发布分支的 PR、检查结果和 Secrets 状态；合并后在 GitHub Actions Secrets 配置 Cloudflare 凭据，手动同步 `v0.9.0` 并验证 Cloudflare Pages 产生新静态部署、根/版本 manifest、安装包和 `.sig`。真实上传验证完成后继续 v0.9.1 编辑器稳定化，再处理 #88 的 PDF 文件落盘/更新器桌面回归和 #169 的工作区规模基线。#88、#195、#226、#227 仍按实际验收状态管理，不提前关闭；不要重复实现当前的搜索高亮、阅读位置、外部修改保护、WYSIWYG 渲染防抖、工作区入口收敛、启动/编辑/保存/无冲突外部刷新/外部新增删除/HTML+Word 导出/关闭确认 smoke。
