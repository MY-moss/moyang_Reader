...
5. 只有远程状态已确认且前序无真实阻塞时，才从最新 main 建一个 `codex/` 分支，只完成一个垂直切片；当前最早动作是重新核对最新 `main`、预算变化和稳定职责边界，再定义 `D13`，不要跳过任务顺序。
