...
- 当前工程主线是：A14 v0.11.0 已发布 → B01–B06 v0.12 可靠性/真实使用切片完成 → C01–C04 v1.0 Freeze/Compatibility/RC 收口完成 → D00 事实源与接手文档收口完成 → D01 开发环境与接手入口 → D02 开发环境自检完成 → D03 标准首次运行验证路径完成 → D04 v1.0 Freeze 执行契约完成 → D05 架构复杂度预算；A07–A13 的仓库内收口和 v0.12 Exit Gate 证据已完成。
...
5. 只有远程状态已确认且前序无真实阻塞时，才从最新 main 建一个 `codex/` 分支，只完成一个垂直切片；当前最早可执行任务是 `D05`（架构复杂度预算），不要跳过任务顺序。
