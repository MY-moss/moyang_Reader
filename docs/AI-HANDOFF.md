...
- 当前工程主线是：A14 v0.11.0 已发布 → B05/B06 收口 v0.12 可靠性/真实使用已完成 → v0.12 Exit Gate → C01 兼容矩阵完成 → C02 文件异常恢复矩阵完成 → C03 发布链路与安全披露收口完成 → C04 v1.0 RC 稳定化；A07–A13 的仓库内收口和 B01–B04 可靠性切片已完成。
...
5. 只有远程状态已确认且前序无真实阻塞时，才从最新 main 建一个 `codex/` 分支，只完成一个垂直切片；当前最早可执行任务是 `C04`，不要跳过任务顺序。
...
- #241：已在隔离 Windows x64 环境完成 v0.10.14 → v0.11.0 安装器覆盖升级，以及应用内 updater 的检查、下载、安装、重启 smoke；升级后的正式构建也已接收 PDF 命令行入口并留下阅读记录，但 PDF 页面内容仍需人工/可视化确认，CI 不能替代这项实机证据。
