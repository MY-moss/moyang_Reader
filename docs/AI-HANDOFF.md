...
- 当前工程主线是：A14 v0.11.0 已发布 → B05/B06 收口 v0.12 可靠性/真实使用已完成 → v0.12 Exit Gate → C01 兼容矩阵完成 → C02 文件异常恢复矩阵 → C03/C04 v1.0 RC 收口；A07–A13 的仓库内收口和 B01–B04 可靠性切片已完成。
...
5. 只有远程状态已确认且前序无真实阻塞时，才从最新 main 建一个 `codex/` 分支，只完成一个垂直切片；当前最早可执行任务是 `C02`，不要跳过任务顺序。
