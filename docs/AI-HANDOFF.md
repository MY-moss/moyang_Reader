...
- 目标：让用户在恢复草稿前看清当前版本与草稿的差异、草稿保存时间和恢复风险，避免因为“不知道差异”而误恢复。
- 已实现：新增轻量逐行差异摘要和有限预览；当前文档优先与磁盘基线比较，其他文档与草稿保存时的原文比较；恢复中心和当前文档提示均先进入差异对话框；恢复只替换编辑区，必须显式保存才写回文件；当前编辑区有未保存内容或磁盘在草稿后发生变化时明确告警；内容已与基线相同的旧草稿不再显示为可恢复项。
- 验收：差异统计显示新增/移除行与字符变化；预览最多 80 行并提示截断；取消/按 Escape 不恢复；恢复动作具有明确文案；当前草稿和非当前草稿均能进入对应预览。
