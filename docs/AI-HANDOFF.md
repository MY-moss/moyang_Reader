...
## 进行中的 v0.10.6 候选切片（2026-08-28）

- 目标：在 #232 已有文件树 CRUD 基础上增加复制文件/文件夹、复制相对路径、文件夹展开/折叠，以及源文本/WYSIWYG 的清除格式、任务列表和日期插入。
- 基线：`origin/main@7f5a26023a5e23020e6026fbbe9725712668005f`；功能分支：`codex/context-menu-actions-2026-08-28`。
- 非目标：移动、回收站撤回、批量复制、版本历史、云同步、DOCX/PDF 原格式回写；不新增运行时依赖。
- 验收：前端全量单测、Rust 全量单测、lint、format、构建、PR Quality checks 和 Windows 定向桌面回归；复制命令必须拒绝越界路径、目标重名和符号链接。
- 发布边界：这是 v0.10.6 patch 候选；仅在 PR 合并且稳定发布检查通过后生成 Windows x64 安装包、`.sig`、`latest.json` 和镜像资产。
- 当前状态：本地代码已完成并通过前端 211/211、Rust 43/43、lint、format 和 build；Windows 定向右键管理 smoke 1/1 通过。完整桌面 smoke 首次运行 9 项通过、右键用例因副本名称的模糊测试定位失败，已改为按条目名称精确定位后定向复验通过；PDF 文件导出仍因“保存 PDF 失败”受 #241 影响。待提交、推送、创建 PR、门禁验证和发布决定。
