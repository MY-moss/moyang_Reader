...
- 本切片（#170）将文档内搜索普通命中和当前命中收敛为 `--search-hit` / `--search-hit-active` 主题令牌，系统暗色（无 `data-theme`）和显式深色共用暗色值；新增浏览器 E2E 验证系统暗色下的两个计算背景色。版本文件已统一到 `0.8.3`，目标为重要视觉缺陷 patch；Release、安装包、签名、`latest.json` 和镜像仍待合并后触发并在线核验。
...
- 下一位 AI 的唯一下一步：合并本切片后创建 `v0.8.3` 标签，验证 GitHub Release、安装包、签名、`latest.json`、Cloudflare 镜像和旧版本更新链路，再把本文件的发布结果补成事实；完成后优先选择 #185/#186 的数据安全与键盘交互闭环，或 #88 的 PDF 文件落盘/更新/双链与 `/` 真实桌面回归，或 #169 的工作区规模基线。#170 的代码与回归已完成但在 Release 在线核验前不要关闭 Issue；#180 已由 PR #210 合并并关闭；#177 的防抖实现已由 PR #208 合并并关闭。不要重复实现当前的搜索高亮、阅读位置、外部修改保护、WYSIWYG 渲染防抖、启动/编辑/保存/无冲突外部刷新/外部新增删除/HTML+Word 导出/关闭确认 smoke，也不要把 #174（React 错误边界）混入本切片。
