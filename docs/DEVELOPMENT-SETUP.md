...
`verify:dev` 先运行只读的 `doctor`，再执行工作流测试、Vitest、Lint、架构边界检查和生产构建。它不会安装工具、修改系统设置、执行 `npm install`、启动交互式桌面窗口或运行昂贵 benchmark；失败项表示当前不能可靠进行对应的桌面开发，Git 未提交改动只显示为警告。
...
# 运行轻量首次开发验证（包含 doctor、测试、Lint、架构检查和 Build）
npm run verify:dev
...
- `npm run doctor` 或 `npm run verify:dev` 报错：按失败项补齐对应工具后重新运行；这些命令不会替你安装或修改系统配置。
