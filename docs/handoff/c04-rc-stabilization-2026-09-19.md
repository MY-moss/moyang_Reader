...
- 以临时 PDF 文件作为命令行打开入口启动 v0.11.0，应用进程保持响应；临时 PDF 和测试进程已清理。该入口随后在升级后的应用数据中留下 PDF 的最近文件、打开标签和阅读时长记录，证明路径已被应用接收并进入阅读流程，但仍不把它扩大解释为页面内容的可视化确认。

## 应用内 updater 与 PDF 入口补充证据

- 测试对象：隔离目录 `%TEMP%\\moyang-c04-old-upgrade-20260919\\old-install` 中的已安装 v0.10.14 正式构建；目标为已发布 GitHub Release `v0.11.0`。
- 使用真实桌面 WebDriver 操作正式构建：关闭首次启动提示，打开顶部“更多”，点击“检查应用更新”；界面显示可用版本 `v0.11.0` 后点击“下载并安装”。
- 下载/安装阶段旧 WebView 关闭，WebDriver 会话随旧窗口结束；没有把随后无法继续定位旧窗口误记为“重启应用”按钮已点击。
- 会话结束后的独立事实核验：安装目录文件 `moyang-reader.exe` 的 `FileVersion` / `ProductVersion` 为 `0.11.0`，卸载注册信息为 `0.11.0`，同一安装目录存在重新启动的 `moyang-reader.exe` 进程。因此本次真实闭环覆盖到安装并重新启动。
- 重启后的 v0.11.0 通过真实进程参数打开临时 PDF；应用数据记录了该 PDF 的最近文件、打开标签和阅读时长，且没有产生预览错误提示。该记录足以证明 PDF 入口被接收并进入阅读流程，但当前机器仍未完成对页面内容的人工/可视化确认。
- 结论：应用内 updater 的检查 → 下载 → 安装 → 重启边界已取得实机证据；#241 仍只剩 PDF 页面内容的可视化读取确认，C04 保持 `IN_PROGRESS`。

## 外部边界

- #241：安装器覆盖升级和应用内 updater 的检查、下载、安装、重启 smoke 已完成；仍需要人工/可视化确认 PDF 页面内容，CI 不能替代这项实机证据。
