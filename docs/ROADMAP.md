...
当前稳定基线：`v0.10.7`，下一 Ready 事项：#241 旧版本自动更新器实机回归。Moyang Reader 当前只支持 Windows x64 桌面版；v0.9.5 已发布三栏导航、侧栏滚动和窄屏布局修复，以及可调宽面板交互。#104 的索引快速路径、ASCII 子串候选、工作区文件列表缓存、文件级 LRU、正式 5000 文档 P95 与长文档回退验收已在 PR #257、#259、#261、#264、#266 完成并关闭 Issue；v0.10.1 已完成 Windows x64 稳定发布。撤回/重做阅读位置修复已由 PR #262 合并并随 v0.10.1 发布。PR #269 已合并 v0.10.2 的首个 UI 切片，PR #270 已完成版本准备并发布 Windows x64 安装包；PR #272 已合并首次使用教程和设置持久化修复，v0.10.3 已完成 Windows x64 Release、安装包和 manifest 核验；#187 的完整响应式断点验收仍保持 open。v0.10.4、v0.10.5 和 v0.10.6 均已完成 Windows x64 Release、安装包和 GitHub 资产发布。GitHub 与 Cloudflare 动态公开资产已核验可用，但静态镜像部署仍因仓库 Actions Secrets 未配置而失败。Windows PDF 文件落盘修复已随 v0.10.7 发布；#241 的旧版本自动更新器实机回归仍是下一 Ready 事项。每个版本可以包含多个功能切片；达到一个用户功能版本的验收标准后必须生成安装包和公开更新，不为每个 commit 创建 Release。

## v0.10.7 已发布：Windows PDF 文件落盘修复（#241）

- 目标：修复 Windows 桌面导出 PDF 时 Edge 退出码为 0 但没有落盘文件的问题。
- 结果：PR #284、版本准备 PR #285 已合并；v0.10.7 已生成并发布 Windows x64 安装包、签名和 latest.json。
- 验收：真实 Windows Desktop smoke、Rust 44 项测试、Release Quality gate 和在线 GitHub/Cloudflare 资产核验通过。
- 镜像：动态 Pages 镜像已反映 v0.10.7；静态上传 job 仍需维护者配置 Cloudflare Secrets，客户端保留 GitHub 回退。

## 下一 Ready 事项：#241 旧版本自动更新器实机回归

- 目标：使用旧版安装实例验证更新提示、下载、签名校验、安装替换和自动重启。
- 非目标：不修改已验证的 PDF 导出逻辑，不引入用户文档云同步或新的跨平台支持。
- 验收：旧版可发现 v0.10.7 或后续版本，下载并完成签名校验，安装后应用启动到新版本；失败时保留旧版本且有明确错误。
- 发布：完成实机回归并更新 #241 后，再决定是否需要更新器修复 patch。
## 平台范围收敛
