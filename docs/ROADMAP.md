...
当前稳定基线：`v0.10.3`，当前发布候选：`v0.10.4`，当前开发切片：`v0.10.5` 文件/文件夹管理与正文编辑上下文。Moyang Reader 当前只支持 Windows x64 桌面版；v0.9.5 已发布三栏导航、侧栏滚动和窄屏布局修复，以及可调宽面板交互。#104 的索引快速路径、ASCII 子串候选、工作区文件列表缓存、文件级 LRU、正式 5000 文档 P95 与长文档回退验收已在 PR #257、#259、#261、#264、#266 完成并关闭 Issue；v0.10.1 已完成 Windows x64 稳定发布。撤回/重做阅读位置修复已由 PR #262 合并并随 v0.10.1 发布。PR #269 已合并 v0.10.2 的首个 UI 切片，PR #270 已完成版本准备并发布 Windows x64 安装包；PR #272 已合并首次使用教程和设置持久化修复，v0.10.3 已完成 Windows x64 Release、安装包和 manifest 核验；#187 的完整响应式断点验收仍保持 open。`v0.10.4` tag 已指向最新 `main`，Release workflow 正在生成 Windows x64 资产，Release 尚未以在线核验结果记账。GitHub 与 Cloudflare 动态公开资产已核验可用，但静态镜像部署仍因仓库 Actions Secrets 未配置而失败。每个版本可以包含多个功能切片；达到一个用户功能版本的验收标准后必须生成安装包和公开更新，不为每个 commit 创建 Release。
...
| v0.10.4 | 发布候选：工作区新建笔记/文件夹、空目录持久显示、文件夹/根目录右键菜单，以及 Markdown/TXT 编辑上下文菜单；版本文件已同步，待 tag、Release 和 Windows x64 资产验证                                                                                                  | #232、用户反馈                                                  |
| v0.10.5 | 当前开发：文件/文件夹右键打开、重命名、删除、资源管理器定位、路径复制；重命名/删除后的标签与最近记录同步；Markdown/TXT 编辑菜单补齐撤销、重做、剪切、复制、粘贴、全选；Windows 授权和路径安全校验                                                                  | #232、用户反馈                                                  |
...
- 发布：代码、测试、文档随一个 PR 推送；合并并完成 Windows x64 质量门禁后，作为下一个稳定 patch 发布 `v0.10.5` 安装包、签名、manifest 和镜像资产。当前分支不预先创建 Release。
