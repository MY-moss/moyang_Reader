...
当前稳定基线：`v0.9.5`。Moyang Reader 当前只支持 Windows x64 桌面版；v0.9.5 已发布三栏导航、侧栏滚动和窄屏布局修复，以及可调宽面板交互。#104 的未变化索引快速路径、ASCII 子串候选、工作区文件列表缓存和文件级 LRU 淘汰已合并到当前 `main@55be1887fb6449344e4d9441dc5b11649257de7d`；正式 5000 文档 P95 与长文档回退验收已在当前功能分支完成，待合并后关闭 #104 并评估 v0.10.1 稳定发布。撤回/重做阅读位置修复已由 PR #262 合并，`v0.9.6` patch 安装包仍待稳定发布批次决定。GitHub 与 Cloudflare 公开资产已核验一致，但 Cloudflare 自动镜像凭据仍由 #241 跟踪。每个版本可以包含多个功能切片；达到一个用户功能版本的验收标准后必须生成安装包和公开更新，不为每个 commit 创建 Release。
...
| v0.10.1 | 进行中：#104 的索引快速路径、ASCII 子串候选、工作区文件列表缓存、文件级 LRU 淘汰及正式验收已完成当前实现；待功能 PR 合并后关闭 #104，再发布稳定安装包                                                                                                                            | #104                                                            |
...
- 已完成正式验收：5000 篇每篇至少 2 KiB、混合中英文的文档在暖缓存查询中执行 20 次 P95 计时；本机记录为 38 ms，测试门槛为 P95 <100 ms。超出单文件索引 token 上限的长文档保留在线性回退集合，短文档仍可使用倒排索引，索引不会被整体关闭。
- 本切片只增加验收回归测试和交接记录，不改变搜索结果、用户文档格式、依赖或运行时产品功能；功能 PR 合并后可关闭 #104。
- 发布边界：本切片属于 v0.10.1 的性能验收，不单独创建 Release 或安装包；#104 合并后另开稳定发布切片，统一评估 v0.9.6 与 v0.10.1 的 Windows x64 安装包、更新 manifest 和镜像资产。
