...
当前稳定基线：`v0.9.5`。Moyang Reader 当前只支持 Windows x64 桌面版；v0.9.5 已发布三栏导航、侧栏滚动和窄屏布局修复，以及可调宽面板交互。#104 的未变化索引快速路径、ASCII 子串候选和工作区文件列表缓存已合并到 `main@dbac7224d1ff188116cf52a9ac441c231ab83b3f`；本轮功能分支继续补齐 posting 预算下的文件级 LRU 淘汰，5000 文档正式 P95 和长文档验收仍在 v0.10.1 路线中。撤回/重做阅读位置修复已由 PR #262 合并，`v0.9.6` patch 安装包仍待稳定发布批次决定。GitHub 与 Cloudflare 公开资产已核验一致，但 Cloudflare 自动镜像凭据仍由 #241 跟踪。每个版本可以包含多个功能切片；达到一个用户功能版本的验收标准后必须生成安装包和公开更新，不为每个 commit 创建 Release。
...
| v0.10.1 | 进行中：#104 未变化索引快速路径、ASCII 子串候选、工作区文件列表缓存和文件级 LRU 淘汰已完成；继续完成正式 5000 文档 P95 与长文档性能验收                                                                                                                            | #104                                                            |
...
- 本轮功能分支已完成：索引达到 posting 预算时按文件最近使用顺序淘汰旧 posting，并将被淘汰文件保留在线性回退集合；搜索候选命中会更新文件 LRU，持久化索引同步记录访问序列并升级缓存版本。未改变搜索结果或引入新依赖。
...
- 未完成：正式 5000 文档 P95 和长文档性能验收；#104 保持 open，完成稳定用户价值验收后再纳入版本发布。
