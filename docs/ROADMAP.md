...
当前稳定基线：`v0.10.5`，下一开发目标：`v0.11.0`。Moyang Reader 当前只支持 Windows x64 桌面版；v0.9.5 已发布三栏导航、侧栏滚动和窄屏布局修复，以及可调宽面板交互。#104 的索引快速路径、ASCII 子串候选、工作区文件列表缓存、文件级 LRU、正式 5000 文档 P95 与长文档回退验收已在 PR #257、#259、#261、#264、#266 完成并关闭 Issue；v0.10.1 已完成 Windows x64 稳定发布。撤回/重做阅读位置修复已由 PR #262 合并并随 v0.10.1 发布。PR #269 已合并 v0.10.2 的首个 UI 切片，PR #270 已完成版本准备并发布 Windows x64 安装包；PR #272 已合并首次使用教程和设置持久化修复，v0.10.3 已完成 Windows x64 Release、安装包和 manifest 核验；#187 的完整响应式断点验收仍保持 open。v0.10.4 已完成 Release、Windows x64 安装包和 GitHub 资产发布；v0.10.5 已完成右键文件管理切片、版本准备、Windows x64 Release、安装包和 manifest 在线核验。GitHub 与 Cloudflare 动态公开资产已核验可用，但静态镜像部署仍因仓库 Actions Secrets 未配置而失败。每个版本可以包含多个功能切片；达到一个用户功能版本的验收标准后必须生成安装包和公开更新，不为每个 commit 创建 Release。
...
| v0.10.3 | 已发布：首次使用教程、设置保存状态和 Windows 应用配置文件兜底已由 PR #272 合并；PR #273 完成版本准备并发布 Windows x64 patch 安装包，GitHub/Cloudflare manifest 和下载资产已核验                                                                                   | 用户反馈、设置持久化                                            |
| v0.10.4 | 已发布：工作区新建笔记/文件夹、空目录持久显示、文件夹/根目录右键菜单，以及 Markdown/TXT 编辑上下文菜单                                                                                                                                                             | #232、用户反馈                                                  |
| v0.10.5 | 已发布：文件/文件夹右键打开、重命名、删除、资源管理器定位、路径复制；重命名/删除后的标签与最近记录同步；Markdown/TXT 编辑菜单补齐撤销、重做、剪切、复制、粘贴、全选；Windows 授权和路径安全校验；已提供 Windows x64 安装包、签名、`latest.json` 和动态镜像验证 | #232、用户反馈 |
...
- 目标：把文件树右键菜单扩展为轻量文件管理入口，并让正文编辑器的系统编辑动作与应用级撤销历史一致。
- 范围：文件/文件夹打开、重命名、删除、资源管理器定位、复制完整路径；安全校验；打开标签、最近记录、当前文档和工作区索引的路径同步；Markdown/TXT 编辑器的撤销、重做、剪切、复制、粘贴、全选。
- 非目标：跨目录拖拽移动、批量选择/批量重命名、回收站恢复、云同步、插件市场和非 Windows 发布。
- 发布：代码、测试、文档已由 PR #278 合并到 `main@69819064a261bf411ea98f36f9f3b901548e7175`；版本准备分支将统一更新版本号并在 main CI 通过后发布 `v0.10.5` 安装包、签名、manifest 和镜像资产。
