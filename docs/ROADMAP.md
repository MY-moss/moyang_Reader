...
- 稳定基线为 `v0.10.9`；工作区转移与纯文本粘贴切片已在 `codex/context-menu-transfer-2026-08-28` 实现，待 PR 门禁通过后作为 `v0.10.10` Windows x64 稳定批次发布。
...
- 发布：PR #292 已合并，GitHub Quality checks 全部通过；v0.10.9 Windows x64 Release 已公开。工作区转移与纯文本粘贴作为 `v0.10.10` 独立稳定批次发布。
...
- 发布：代码、定向测试和工程文档随一个 PR 交付；Quality checks 全绿并合并后，更新版本号、生成 Windows x64 安装包、签名、`latest.json`，再核验 GitHub Release 和 Cloudflare 镜像。
...
- 当前 Ready：完成 `codex/context-menu-transfer-2026-08-28` 的 PR、合并和 `v0.10.10` Windows x64 发布；发布完成后停止，不自动开始下一个功能。
