...
`D11` 已完成 `App.tsx` 设置与偏好生命周期编排提取；下一步重新核对最新 `main`、预算变化和稳定职责边界，再定义 D12。具体范围以 [`AI-TASKS.md`](AI-TASKS.md) 为准。在 D00–Dxx 收口前，新产品想法可以记录到 Issue/长期计划，但默认只分析，不编码、不创建实现 PR。
