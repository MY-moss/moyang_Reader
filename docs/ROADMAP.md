...
## v0.10.12 开发切片：大工作区边界与文件树虚拟化（#169，进行中）
...
- 发布：本切片先进入一个独立 PR；稳定批次验收后再决定是否发布 `v0.10.12` Windows x64 安装包，不在开发中重复打包。

## Issue 清理与修复顺序（2026-08-29）

- 审计快照：36 个开放 Issue、0 个开放 PR。以下是本轮审计排序，不改变 GitHub Issue 的 open/closed 状态，也不把所有问题塞进一个 PR。
- 当前切片：#169。先完成大工作区枚举边界与文件树虚拟化；完成质量门禁、PR 合并后，再决定是否以 `v0.10.12` Windows x64 稳定批次发布。
...
1. **#169（当前）**：大工作区文件树的边界、排序和按需渲染；非目标是更换搜索引擎或重做三栏 UI；测试级别为针对性单测、5000 文档性能基准、浏览器 E2E 和一次 Windows smoke。
2. **#168**：长文档滚动与目录定位的稳定渲染；非目标是编辑器模型改造或新增插件语法；测试级别为渲染/交互定向测试、长文档基准和一次 Windows smoke。
3. **#181**：出链索引和编辑器输入路径的重复计算；非目标是重做搜索索引或 Markdown 编辑器架构；测试级别为针对性性能测试和一个编辑器 E2E。

完成一个切片后必须停止并交接；不得因为 Issue 数量多而跳过验收、重复查询 CI 或连续生成开发安装包。

## 当前稳定基线（2026-08-29）

- 稳定基线为 `v0.10.11`；PR #303 已合并，Windows x64 Release、安装包、签名和 `latest.json` 已完成在线核验。
