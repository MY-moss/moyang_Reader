...
- 稳定基线：`v0.10.13`；当前远程主线为 `main@16180304cf80d713ae411015b3832fa4338e96d3`，PR #377 已完成 #357，PR #380 已完成 #358，PR #379 与 PR #381 已完成构建缓存稳定性补强，PR #384 已完成 #360，#369 正在独立分支实现。
...
| 14   | #369     | 回收站删除与保存上一版本保护（实现中）     | Windows 回收站失败边界、`.moyang.bak` 滚动、差异预览恢复和 desktop smoke   |

## #369 回收站删除与保存上一版本保护（实现中）
...
- 验收：前端定向 14 项、Rust 备份与删除测试、工作流 13/13、Windows desktop smoke 14/14、Lint、格式和构建已通过；远程 Quality checks 和 PR 尚待创建。
- 风险：特殊路径、网络盘或系统禁用回收站时 Shell 操作可能失败；应用不会静默退回永久删除。发布前需在目标 Windows 环境手动确认 Explorer 还原路径。
- 发布：不单独生成安装包或 Release；合并后纳入 `v0.11.0` 稳定 Windows x64 批次。回滚为回退本切片 PR。
