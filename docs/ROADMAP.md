...
A07–A13 DONE
  ...
  → D11 App.tsx settings orchestration IN PROGRESS
  → v1.0 final RC
  → v1.0
```

`D11` 当前正在提取 `App.tsx` 设置与偏好生命周期编排；具体范围以 [`AI-TASKS.md`](AI-TASKS.md) 为准。在 D00–Dxx 收口前，新产品想法可以记录到 Issue/长期计划，但默认只分析，不编码、不创建实现 PR。
