...
- 审计快照：35 个开放 Issue、0 个开放 PR。以下是本轮审计排序，不改变 GitHub Issue 的 open/closed 状态，也不把所有问题塞进一个 PR。
- 当前切片：#168 长文档滚动阅读轨道正在开发；#169 和 #307 已完成，发布结果已核验。完成 #168 后重新检查 Issues，再从 Ready backlog 选择下一项。

| 优先级                 | Issue                                                                  | 处理顺序与边界                                                                                                                                      |
| ---------------------- | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| Must / 核心性能        | #179、#169、#168、#181、#182、#165                                     | 工作区 IO、文件列表规模、长文档滚动、编辑器重渲染、文内搜索高亮和 WYSIWYG 回归测试；#179 已完成，当前完成 #169 后再按 Ready 顺序处理 #168 或 #181。 |
...
1. **#168（下一候选）**：长文档滚动与目录定位的稳定渲染；非目标是编辑器模型改造或新增插件语法；测试级别为渲染/交互定向测试、长文档基准和一次 Windows smoke。
2. **#181**：出链索引和编辑器输入路径的重复计算；非目标是重做搜索索引或 Markdown 编辑器架构；测试级别为针对性性能测试和一个编辑器 E2E。
3. **#182**：编辑器重渲染与输入响应边界；非目标是重新设计编辑器数据模型；测试级别为编辑器定向测试、性能基准和一次 Windows smoke。

完成一个切片后必须停止并交接；不得因为 Issue 数量多而跳过验收、重复查询 CI 或连续生成开发安装包。

## #168 实施记录：长文档滚动阅读轨道（2026-08-29）
...
- 当前验证：相同场景产生 1 次标题查询和 284 次布局读取；布局读取仅发生在跳跃滚动无法由观察器直接确定标题的回退路径，不再随每个滚动帧扫描全部标题。浏览器 E2E 增加性能回归阈值。
- 边界：不改 Markdown 渲染、目录数据结构、编辑器模式、阅读位置持久化或导出；不引入新依赖；当前仍需在远程 Quality checks 和 Windows 桌面 smoke 中复核。
- 发布：本切片不单独生成稳定安装包；待 #168 与已合并的 #307 汇入下一稳定补丁批次后，再按发布政策生成 Windows x64 安装包、签名、manifest 并核验镜像。
