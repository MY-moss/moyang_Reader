...
- 稳定基线为 `v0.10.8`；v0.10.9 右键快速管理补全已由 PR #292 合并到 `main@cf713ab97b48dd18c872b2ad1b2fcbf12cd52bc4`，下一步是独立的 Windows x64 发布准备、安装包、Release、manifest 和镜像核验。
...
- 非目标：文件移动、回收站恢复、批量文件操作、属性写回、DOCX/PDF 原格式编辑和云同步继续单独评估。
- 验收：根目录/文件夹/文件的刷新范围正确；文件树 CRUD、属性、路径和标签不回归；阅读模式双链、锚点、本地和外部链接复用正文点击逻辑；定向单测、浏览器 E2E、类型与格式检查通过。
- 发布：PR #292 已合并，GitHub Quality checks 全部通过；本功能 PR 未创建 Release 或安装包。下一独立发布切片再更新 v0.10.9 版本号和 CHANGELOG，生成安装包、签名、`latest.json`，并核验 GitHub Release 与 Cloudflare 镜像。
...
- 当前切片已交付：复查 #232 的验收和 PR 门禁已完成；下一 Ready 事项是独立的 v0.10.9 Windows x64 版本准备与发布，不扩展文件移动或批量操作。
- 发布收尾：v0.10.8 的 Cloudflare 静态镜像仍需维护者配置 `CLOUDFLARE_API_TOKEN`（Pages 写入权限）和 `CLOUDFLARE_ACCOUNT_ID` 后重跑；该动作与 v0.10.9 产品切片分开执行。
