...
当前稳定基线：`v0.9.5`。Moyang Reader 当前只支持 Windows x64 桌面版；v0.9.5 已发布三栏导航、侧栏滚动和窄屏布局修复，以及可调宽面板交互。#104 的未变化索引快速路径已合并，5000 文档基准仍在 v0.10.1 路线中。GitHub 与 Cloudflare 公开资产已核验一致，但 Cloudflare 自动镜像凭据仍由 #241 跟踪。每个版本可以包含多个功能切片；达到一个用户功能版本的验收标准后必须生成安装包和公开更新，不为每个 commit 创建 Release。
...
| v0.10.1 | 进行中：#104 未变化索引快速路径已合并；继续完成 5000 文档基准、索引上限和长文档性能验收                                                                                                                                                                            | #104                                                            |
...
- 已验证：针对性 Rust 测试、完整 Rust 测试 37/37、Rust fmt、clippy 和 PR Quality checks 通过；未引入新依赖。
- 未完成：5000 文档基准、索引上限和未入索引文件的进一步命中策略；#104 保持 open，后续完成稳定用户价值验收后再纳入版本发布。
