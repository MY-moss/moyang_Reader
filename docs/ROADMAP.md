...
| v0.9.1  | 编辑器保真、桌面 E2E、a11y 和 i18n 稳定化           | #88、#119、#111、#156、#157、#158  |
| v0.9.2  | 编辑器交互深化：`/` 块级触发器、序列化风格偏好、e2e 防御性回退、桌面端 `[[` 补全手动验证收尾 | #157、#158、#156 |
...
3. **键盘流与焦点管理**：编辑器内 `/` 触发块级菜单（标题/列表/表格/代码块/引用）；命令面板支持拼音前缀与最近使用；全局快捷键一致性审计（同一动作在不同视图快捷键一致）。
