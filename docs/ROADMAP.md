...
- 稳定基线：`v0.10.13`；当前远程主线为 `origin/main@4a60c61fe5ec0b6adc760f177af4f166275984e`，PR #377 已完成 #357，PR #380 已完成 #358，PR #379 与 PR #381 已完成构建缓存稳定性补强，PR #384 已完成 #360，PR #386 已完成 #369；#359 当前在独立分支实现。
...
| 15   | #359     | 撤销历史分组与内存上限（进行中）           | 400ms 输入分组、100 条/8 MiB 历史预算、跨模式撤销回归和定向浏览器 E2E 通过        |
...
- 验收：编辑历史定向单测 6/6、TypeScript、一次生产构建、源码快速输入 E2E 1/1、跨模式撤销/重做 E2E 1/1，以及最终 lint、format 和 diff 检查均已通过，可进入本地 checkpoint。
