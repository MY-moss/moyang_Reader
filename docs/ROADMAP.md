...
- 稳定基线为 `v0.10.8`；当前开发切片是 `v0.10.9` 右键快速管理补全（#232）。合并后再执行 Windows x64 稳定批次的版本准备、安装包、Release、manifest 和镜像核验。
...
- 发布：合并并完成 Windows x64 稳定批次门禁后，更新 v0.10.9 版本号和 CHANGELOG，生成安装包、签名、`latest.json`，再分别核验 GitHub Release 与 Cloudflare 镜像；本 PR 阶段不重复生成安装包。
...
- 当前切片交付后：复查 #232 的验收和 PR 门禁，合并后准备 v0.10.9 Windows x64 安装包/Release；不在同一 PR 内继续扩展文件移动或批量操作。
