...
- 当前状态：READY（#368 文档书签第一切片）。
- 当前主线基线（#367 已完成）：`main@556a6fa98c7a5afd69aa88d4d2167ce5a90f6f8b`；#367 已通过 PR #395 合并并完成交接。
...
- #368 完成验证、PR、Issue 状态和交接同步后停止；下一项必须重新检查 Issues。
...
- Issue：[#368](https://github.com/MY-moss/moyang_Reader/issues/368)；状态：Ready 候选；计划版本：`v0.11.x`，不单独发布。
...
- 预计范围：`src/app/bookmarks.ts`、`ReaderContextMenu.tsx`、`ContextPanel.tsx`、存储边界、对应单测和一条书签回归 E2E；先从最新 `main` 建项目内 worktree。
- 风险与回滚：路径大小写、重命名和失效锚点必须显示可解释状态，不得静默丢失；回滚为回退本切片 PR，保留未知 localStorage 数据。
