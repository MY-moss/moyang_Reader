...
- [x] 长文档滚动到中部打开链接、图片和表格插入，浮层完整位于视口内。
- [x] 打开浮层不会改变 `.content-area` 的 scrollTop，输入框获得焦点也不触发滚动回文首。
- [x] 编辑器滚动后浮层关闭或重新定位，不残留过期锚点；Escape/外点关闭可继续编辑。
- [x] 提交后原编辑器选区/光标恢复，既有 Markdown 输出和右键菜单焦点契约不回归。
- [x] 相关单测、lint、format、TypeScript 检查和一个浏览器 E2E 通过。
