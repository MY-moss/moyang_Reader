...
- [ ] **#370 阅读历史与本地统计（步骤 1/3）**：从已合并的 `main@f064b4621232dae9cbb292f6eaf200b5e3a3604a` 创建项目内独立工作树；本轮记录最近文档 `lastOpenedAt`，兼容无时间戳旧数据，将上限提升至 50，按时间排序并显示相对日期。PR [#420](https://github.com/MY-moss/moyang_Reader/pull/420) 已上传，Quality checks run `33611774822` 全部通过，待合并；验收包含存储迁移/排序单测、最近列表组件覆盖和至少一条浏览器 E2E；不实现阅读时长心跳、统计面板或清理入口。

## 下一批候选（完成 #370 当前切片后重新检查，不自动启动）

按“用户可见收益 / 无外部阻塞 / 可独立验收”排序：

1. **#370 后续步骤**：按 Issue #370 单独拆分阅读可见性心跳与本周统计；不与当前最近列表切片混做。
