...
- [x] **#416 Windows 外部图标、快捷方式与文件关联一致性**：从 `main@b11539ea85bc816dbb9f002021084755d7c826b2` 创建项目内工作树；已完成资源检查、Windows 桌面验证和升级验证，待一个 PR/CI 与交接完成后停止。

## 下一批候选（完成 #416 后重新检查，不自动启动）

按“用户可见收益 / 无外部阻塞 / 可独立验收”排序：

1. **#366 统一确认弹层**：减少误操作和文案歧义；T2；组件测试 + 一个 Windows 关闭路径。
2. **#370 阅读历史与本地统计**：让最近列表真正有时间语义；T1/T2；仅本机数据。
3. **#233 顶栏图标与操作密度**：清理右上角重复/低频入口，改善窄窗口；T2；不与大范围主题重构混做。

条件旁路（不抢占普通切片）：#241 更新/镜像/PDF 实机矩阵、#51 Authenticode、#112 更新文档；只有外部环境满足时才改为 READY。

## Must / 稳定性

- [x] #416 图标一致性（已完成实现与本地验收，待 PR/CI）。
