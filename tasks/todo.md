...
- [x] **G-02 发布/交接状态结构化检查（已实现）**：新增只读 `release:status` 与 `docs/release-status.json`，校验版本、唯一 NEXT 状态、CHANGELOG、Windows x64 Release 资产、镜像/外部阻塞和交接链接；分支 `codex/g02-release-state-2026-09-03`，PR 待创建，基线 `main@c8884859068642705761d6b398dbef869fddfc9e`。
...
- 验收：默认仍为只读预览；预算阈值和闲置时间提示可由测试固定；活动/受保护 target 只报告不删除；超过预算给出明确路径、大小和建议；定向工作流测试、Lint、格式和 `git diff --check` 通过。
- 风险/回滚：误判闲置时间或预算可能导致错误建议；用确定性 fixture、只读 dry-run 和受保护路径测试控制，回退 G-03 PR 即可恢复现有清理预览输出。
- 分支：下一切片启动前从最新 `main@c8884859068642705761d6b398dbef869fddfc9e` 创建独立 `codex/` 分支，并重新核验是否有重复 Issue/PR。
...
1. **G-03 构建缓存预算提示**：只读报告大小/闲置时间和明确清理建议；不自动删除活动 target，不改变 D 盘构建目标路径；T1。
2. **#112 更新与 opener 文档收口**：在不重复实现更新器的前提下补齐权限边界、镜像回退和故障排查；T2/T3。
