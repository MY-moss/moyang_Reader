...
- [ ] **#233 顶栏图标体系与操作密度**：Issue 启动前已重新核验无重复产品 PR；当前主线为 `main@b7dc14358aee7025a83e86a7ba06d865914fddb1`，独立工作树/分支 `codex/topbar-icons-2026-09-02` 增加零依赖统一 stroke SVG，接入撤销/重做、外部修改、侧栏、上下文、搜索、设置与导出；PR [#423](https://github.com/MY-moss/moyang_Reader/pull/423) 等待远程门禁。

## 下一批候选（完成 #233 当前切片后重新检查）

按“用户可见收益 / 无外部阻塞 / 可独立验收”排序：

1. **#191 键盘与读屏导航**：完成 #233 后重新核验 Issue，补树导航、目录高亮、搜索结果、阅读播报和焦点归还；T2；增加 axe/Windows 验收。
