...
- [ ] **#370 阅读历史与本地统计（步骤 2/3）**：步骤 1 的 PR [#420](https://github.com/MY-moss/moyang_Reader/pull/420) 已 squash 合并为 `main@a75ff75e6600d82bb0223741b91a0b1309d9a07a`；当前独立工作树/分支 `codex/reading-heartbeat-2026-09-02` 只实现本机前台可见性心跳、整数阅读秒数和近一年按本地日期持久化。暂停、恢复、关闭和存储异常有覆盖；不实现周统计 UI、清理入口、联网或图表库。

## 下一批候选（完成 #370 当前切片后重新检查）

按“用户可见收益 / 无外部阻塞 / 可独立验收”排序：

1. **#370 步骤 3**：按 Issue #370 单独增加周统计与清理入口；不与当前前台心跳切片混做。
