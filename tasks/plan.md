...
- [x] **F-01 #416 Windows 外部图标一致性（已合并）**：显式 `bundle.icon`、资源完整性检查、全新安装/覆盖升级/快捷方式/文件关联验证；PR #418 已 squash 合并为 `main@45334c0b6cf9dc5f9b1bd39d2803b96181f0643e`，Issue #416 已关闭；不重做 Logo、不做跨平台；T3；不单独发布。
...
- [ ] **F-05 #366 统一确认语义（当前切片，PR #419 CI 全绿待合并）**：清空草稿、关闭窗口和未保存修改使用应用内弹层，说明草稿已自动留存，并提供保存并退出；不处理 `window.prompt` 系列、不改数据模型；组件测试、浏览器 E2E、Windows 关闭路径；T2。
- [ ] **F-06 #370 阅读历史与本地统计**：最近文档有时间语义、阅读时长和清理入口；仅本机、有限条数、无云同步；T1/T2。
- [ ] **F-07 #191 键盘与读屏细节**：树导航、目录高亮、搜索结果、阅读播报和焦点归还；不引入新 UI 框架；T2。
- [ ] **F-08 #233 顶栏图标与密度**：统一当前文档操作、低频菜单和窄窗口溢出；不与完整主题重构混做；T2。
