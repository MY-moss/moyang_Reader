...
- [x] **F-01 #416 Windows 外部图标一致性（已完成实现与本地验收，PR #418 待 CI/审查）**：显式 `bundle.icon`、资源完整性检查、全新安装/覆盖升级/快捷方式/文件关联验证；不重做 Logo、不做跨平台；T3；候选发布 v0.10.15，公开 Release 仍需单独按稳定批次政策执行。
