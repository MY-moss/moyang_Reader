...
- [ ] **F-07 #191 键盘与读屏细节（当前子切片）**：第 1 个子切片已合并快速打开高亮/活动后代同步；当前第 2 个子切片为已打开文档标签栏的 roving tabindex、左右/Home/End 导航和选中同步；文件树、目录高亮、读屏播报和焦点归还继续拆分；不引入新 UI 框架；T2。PR [#425](https://github.com/MY-moss/moyang_Reader/pull/425) 的 Quality checks run `33638296839` 进行中。
