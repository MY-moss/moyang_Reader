...
- [x] **F-09g1 #171 CSS 字体字号令牌治理第三批（已合并）**：顶栏、更多/设置、查找栏、标签栏、左侧阅读库操作与文件条目、阅读历史摘要、状态栏使用 `--type-*` 令牌收敛 9–19px 字号；保持当前计算值和布局行为，不改业务语义；T2；分支 `codex/css-font-token-2026-09-03`，PR [#436](https://github.com/MY-moss/moyang_Reader/pull/436) 已 squash 合并为 `main@9dfe5d8dc806023ab2881c04300d24790e35c167`，Quality checks run `33719109262` 通过。
- [x] **F-09g2 #171 CSS 动效时长令牌治理（本切片，待合并）**：将 `.file-drop-card` 的 140ms 与 `.quick-open-item` 的 130ms transition 时长收敛到语义令牌，保持原有计算值和 reduced-motion 行为；补静态约束与 720/900px 浏览器回归；不与主题、#194、#16 或 HTML 安全路线混做；T2；分支 `codex/css-motion-token-2026-09-03`，PR [#437](https://github.com/MY-moss/moyang_Reader/pull/437)，远端提交 `a1bf9fda50d228f2ba38e647e8dd21adca861236`，等待 Quality checks 与合并。
- [ ] **F-09g3 #171 CSS 主题规则后续边界**：待 F-09g2 合并后重新核验 Issue/PR，只选择一个主题覆盖边界并补静态与浏览器回归；不与 #194、#16 或 HTML 安全路线混做；T2。
