...
- [x] **F-01 #416 Windows 外部图标一致性（已完成实现与本地验收，PR #418 CI 全绿待人工审查）**：显式 `bundle.icon`、资源完整性检查、全新安装/覆盖升级/快捷方式/文件关联验证；不重做 Logo、不做跨平台；T3；候选发布 v0.10.15，公开 Release 仍需单独按稳定批次政策执行。
...
- [ ] **F-05 #366 统一确认语义**：清空草稿、关闭窗口和未保存修改使用同一应用内弹层，说明“草稿可恢复”；不改数据模型；T2。
