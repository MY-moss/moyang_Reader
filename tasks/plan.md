...
- [ ] **F-09g3 #171 CSS 页面背景主题令牌治理（本切片，待合并）**：将 `body` 固定浅色渐变收敛到 `--page-background`，为系统/显式深色提供一致深色底色，并保留 forced-colors 的 `Canvas`；补静态约束与 720/900px 浏览器回归；不与 #194、#16 或 HTML 安全路线混做；T2；分支 `codex/css-theme-token-2026-09-03`，PR 待创建。
