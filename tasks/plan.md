...
- [ ] **F-09g4 #171 CSS 批注/高亮主题令牌化（本切片，待合并）**：新增 `--annotation-border` 与 `--annotation-surface`，统一批注引文、当前批注卡、正文命中和 CSS Highlight 的颜色；补静态约束、浅色/显式与系统深色、forced-colors 和 720/900px 浏览器回归；不与 #194、#16 或 HTML 安全路线混做；T2；分支 `codex/css-theme-followup-2026-09-03`，提交 `fccaafa`，PR [#439](https://github.com/MY-moss/moyang_Reader/pull/439) 等待 Quality checks 与合并。
