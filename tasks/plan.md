...
- [ ] **F-06 #370 阅读历史与本地统计（当前切片：步骤 1）**：本轮只为最近文档增加可选 `lastOpenedAt`、将上限从 12 提升至 50、按有效时间降序显示并提供中文相对日期；旧 localStorage 无迁移报错。阅读可见性心跳、阅读时长、本周统计和清理入口拆为后续独立切片；仅本机、无联网/匿名上报、不引入图表库；T1/T2。
