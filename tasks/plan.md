...
- [ ] **F-07 #191 键盘与读屏细节（当前子切片）**：快速打开、标签栏和文件树三个子切片已合并；当前第 4 个子切片为目录 roving tabindex 与当前章节高亮；读屏播报和焦点归还继续拆分；不引入新 UI 框架；T2。当前分支 `codex/outline-roving-2026-09-03`，PR 待创建。
...
- [ ] **F-07d #191 目录 roving 与当前章节高亮（当前）**：目录使用 `tree/treeitem` 语义、单一 roving Tab 停靠点、上下/Home/End 和现有中央正文导航；当前高亮同步 `active`/`aria-current`/`aria-selected`；读屏播报和 Esc 互斥另行拆分；T2。
