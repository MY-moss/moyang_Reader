...
- [x] **F-09e #171 CSS 颜色令牌治理第一批（待合并）**：错误/警告、代码块/行内代码、文件卡片、状态栏和工作区列表使用语义颜色令牌；自动/显式深色主题移除逐组件重复覆盖；不改布局和业务逻辑；T2；分支 `codex/css-token-governance-2026-09-03`，PR [#434](https://github.com/MY-moss/moyang_Reader/pull/434) 已创建，Quality checks run `33709940611` 和依赖审计 run `33709940595` 已通过；Issue #171 保持开放，后续令牌批次另行拆分。
