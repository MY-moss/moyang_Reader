...
use document::{
    decode_text, document_kind, extract_links, extract_tags, extract_title,
    is_supported_document_path, is_supported_text_path, markdown_extension, IMAGE_EXTENSIONS,
};
...
fn index_entry_for_file(file: WorkspaceFile) -> Option<WorkspaceIndexEntry> {
    ...
    Some(WorkspaceIndexEntry {
        title: extract_title(&source, &file.name),
