...
v0.8.1 是当前稳定基线；v0.9.0 正在以可审查的功能切片推进，继续保持 Windows 优先、本地优先和轻量启动：
...
- Markdown 编辑器支持 `Ctrl+E` 模式切换、`Ctrl+K` 插入链接、`Ctrl+Shift+P` 命令面板；统一 Markdown 渲染链仍负责阅读和导出
...
`src-tauri/target/release/bundle/nsis/Moyang Reader_0.8.1_x64-setup.exe`
...
1. v0.8.1：当前稳定基线，覆盖工作区、搜索、双链、导出、恢复、更新和 Windows 安装包。
