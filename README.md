...
v0.6.0 版本批次已完成多工作区、会话恢复、编辑体验和导出性能升级，继续基于轻量本地阅读核心迭代：
...
`src-tauri/target/release/bundle/nsis/Moyang Reader_0.6.0_x64-setup.exe`
...
1. v0.6.0：当前稳定批次，覆盖多工作区、会话恢复、CodeMirror 编辑器、搜索索引、批量导出性能和关系图修复。
2. v0.7：超大文档增量渲染、持久化索引、崩溃恢复、更完整的打印设置和无障碍自动化验证。
3. v0.8：插件/适配器机制、可选同步、版本回滚提示和跨平台构建前置验证。
4. v1.0：在核心功能和跨平台发布稳定后，再评估同步生态、扩展能力与长期兼容性承诺。
