...
v0.5.4 本地候选版（阅读区与侧栏体验增强），继续基于轻量本地阅读核心完善：
...
- 工作区目录扫描、最近打开、全局全文搜索（最多返回 100 条）；重启后可自动恢复上次阅读库、有效标签页和最后文档
- 已打开阅读库时可从左侧路径旁直接切换最近阅读库，适合在多个本地资料库之间快速往返
- 阅读库支持一键打开当前筛选文档并依次加入标签页；大目录默认最多打开 40 个，避免一次性占用过多资源
...
- 阅读库支持按 Markdown、纯文本、Word、PDF 和图片类型筛选，并联动搜索、批量打开与批量导出
- 侧栏会显示筛选后的文档数量，并可一键清除标签和类型筛选，明确当前打开/导出的范围
- 当前文档一跳关系图，可点击节点打开关联文档
- 多标签页、文件/文件夹打开、批量打开、最近阅读库切换和文档内链接都会在替换未保存内容前确认
...
- 导出菜单提供当前文档打印版式预览，可在打印前确认纸张、方向、页边距和最终排版；主按钮仍保留一键打印 / 保存 PDF
...
`src-tauri/target/release/bundle/nsis/Moyang Reader_0.5.4_x64-setup.exe`
...
2. v0.6：更完整的 PDF 导出模板、页眉页脚、分页预览、批量 PDF 导出和导出附件处理（DOCX 页眉页脚与页码已先行落地）。
